...
## 19.对路由守卫进行封装  
	--19.1 创建 路由守卫的组件 src/components/common/auth/index.js
```
//封装路由守卫,更适合于后台项目,后台项目的特点:除了登录页其余页面都是需要守卫的
import React,{Component} from 'react';
import axios from 'axios';
...
class Auth extends Component{
	...
	render(){
		...
		//找到当前页的path和 component
		let nowRouter = config.find(v=>v.path === pathname);
		console.log(nowRouter);
		
		if(tokenType){
			return <Route path={pathname} component={nowRouter.component} />
		}else{
			return <Redirect to ="/login" />
		}
	}
...
export  default Auth;
	```
	--19.2创建了需要守卫的路由配置文件 src/router/config.js
```
//路由配置
//需要守卫的路由
import Center from '../pages/center';
import Cart from '../pages/cart';
 ...
 export const routerConfig = [
	...
	```
	--19.3 修改了路由文件src/router/index.js
```
//路由
//引入插件
import React from 'react';
...
export default App;
	```
	--19.4修改了center页面 src/pages/center/index.js
```
import React,{Component} from 'react';
import {Redirect} from 'react-router-dom';
import axios from 'axios';
...
## 20.将返回封装为高阶组件的形式
	--20.1 创建高阶组件 src/components/hoc/withGoBack.js
```
import React ,{Component} from 'react';
...
<span onClick={this.props.goBack}>{"<"}</span>
	```
...
## 21.后台接口进行封装
	--21.1 创建封装 axios  src/http/http.js
```
//封装  axios的
import axios from 'axios';
...
export const lunboData = async()=>{
	...
	--21.3 在首页上调用封装好的api  src/pages/main/index.js
```
//引入获取数据的api
